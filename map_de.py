...
gender = st.selectbox('자연 지향 선택', ['도시 선호', '자연 선호'])
age_range = st.slider('연령대 선택', min_value=0, max_value=100, value=(0, 100))
gender_dict = {"남성":"남", "여성":'여'}
prefer_dict = {'city' : '도시 중간선호', 'rural':'자연 중간선호'}#추가 작업

# 필터링된 데이터셋 만들기
filtered_data = data
if gender != '전체':
    filtered_data = filtered_data[filtered_data['자연 vs 도시'] == prefer_dict[gender]]
